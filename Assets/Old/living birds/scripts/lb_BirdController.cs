...
public class lb_BirdController : MonoBehaviour {
	...
	Vector3 FindPointInGroundTarget(GameObject target){
		...
		Vector3 point;
		point.x = Random.Range (collider.bounds.max.x,collider.bounds.min.x);

		point.y = collider.bounds.max.y;
		point.z = Random.Range (collider.bounds.max.z,collider.bounds.min.z);
		//raycast down until it hits the ground
		RaycastHit hit;
		if (Physics.Raycast (point,-Vector3.up,out hit,collider.bounds.size.y,groundLayer)){
			return hit.point;
		}
	...
	void BirdFindTarget(GameObject bird){
		...
		if (birdGroundTargets.Count > 0 || birdPerchTargets.Count > 0){
			...
			for (int i=0;i<birdGroundTargets.Count;i++){
				Collider collider = birdGroundTargets[i].GetComponent<Collider> ();
				gtArea += collider.bounds.size.x*collider.bounds.size.z;
